...
def get_graph() -> TaskGraph:
    ...
    print("micro",microstructures_path)
    workflow = None
...
def standard_task_graph_AND_comp():
    g = get_graph()
    num_tasks = len(g.task_names)

    task_graph_ids = {
                    task: i
                    for i, task in enumerate(g.task_names)
                    }
    ...
    comp = torch.Tensor([
                [
                     g.task_names[reverse_task_graph_ids[i]].cost

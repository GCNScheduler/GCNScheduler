...
class App:
    ...
    def train(self, data, model_config, learning_config, save_path='', mode=NODE_CLASSIFICATION):
        ...
        if mode == NODE_CLASSIFICATION:
            ...
            self.model = Model(g=data[GRAPH],
                               ...
                               is_cuda=learning_config['cuda'],
                               mode=mode)
            #print("-->> inside Train -->>entities and rels",data[N_ENTITIES],data[N_RELS])
    ...
    def CUSTOMIZED_test(self, data,type_data_to_check, load_path='', mode=NODE_CLASSIFICATION):
        ...
        if mode == NODE_CLASSIFICATION:
            test_mask = data[type_data_to_check]
            labels = data[LABELS]
            acc, _ = self.model.eval_node_classification(labels, test_mask)
            ###
            predicted_labels = self.model.get_predicted_labels(labels, test_mask)

...
def workflow_to_forward_graph(workflow: List[Dict[str, Any]]) -> Tuple[Dict[int, List[int]], torch.Tensor, Dict[Tuple[int, int], int]]:
    ...
    ###
    set_ending,set_begining = set(),set()
    for k,v in forward_graph.items():
        set_ending.add(k)
        for value in v:
            set_ending.add(value)
    print("DESTINATIONS",list(set_ending.difference(set_begining)) 
    for dest in list(set_ending.difference(set_begining)):
        forward_graph[new_key]=[]
    ##
...
def get_graph(recipe: str, num_tasks: int) -> Dict[int, List[int]]:
    ...
    print("->get graph",workflow_to_forward_graph(workflow_instance["workflow"]["tasks"]))    
    return workflow_to_forward_graph(workflow_instance["workflow"]["tasks"])
    
if __name__ == "__main__":
    forward_graph, comp, edge_weights = get_graph("montage", 50)
